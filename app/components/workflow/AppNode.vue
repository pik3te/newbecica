...
const accent = computed(() => props.data?.accent ?? (isDark.value ? '#c084fc' : '#6366f1'))
const iconName = computed(() => props.data?.icon ?? 'i-lucide-app-window')
...
const wrapperStyle = computed(() => (
  props.selected ? { '--tw-ring-color': accent.value } as Record<string, string> : undefined
))
...
const dynamicWidth = computed(() => {
  const base = 150
  const charWidth = 10
  return `${Math.max(base, label.value.length * charWidth + 80)}px`
})
...
<template>
  <div :class="wrapperClasses" :style="nodeStyle">
    <Handle
      id="app-target"
      type="target"
